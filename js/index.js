...
function meltdown(board, automaton) {
    for (let y = 0; y < board.height; ++y) {
        for (let x = 0; x < board.width; ++x) {
...
function cycleRules(board, automaton) {
    const nborStates = {};
    for (let y = 0; y < board.height; ++y) {
        for (let x = 0; x < board.width; ++x) {
            const nbors = countNbors(board, automaton.length, x, y);
            if (nborStates[nbors] === undefined) {
                nborStates[nbors] = new Set();
            }
...
window.onload = () => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const cute = yield stbi_load_from_url(imgPath);
